...
// === REPLACED MAP INIT: View locked to Aceh (center + extent) ===
var acehExtentLonLat = [94.5, 1.5, 99.0, 6.8];
var acehExtent = ol.proj.transformExtent(acehExtentLonLat, 'EPSG:4326', 'EPSG:3857');
var acehCenter = ol.proj.fromLonLat([96.5, 4.5]);
...
// Ensure initial fit so Aceh fills the viewport
if (map && map.getView) {
    try {
        map.getView().fit(acehExtent, { size: map.getSize(), padding: [50,50,50,50], maxZoom: 10, duration: 800 });
    } catch (e) {
        // ignore if view size not ready yet
    }
}
// === END REPLACED BLOCK ===
...
map.getView().fit([10576564.564726, 223187.715404, 10941311.304290, 677618.577466], map.getSize());
